...
const STEP_INFO = [
  { title: 'Cabecera', subtitle: 'Fecha, destino, propietario y motivo' },
  { title: 'Detalle',  subtitle: 'Categorías y cantidades a retirar' },
];

type FormErrors = Record<string, string>;
type ItemEntry = { categoriaId: string; cantidad: string };

export default function SalidaCrearScreen() {
  const router = useRouter();
  const { crear, isLoading, motivos, cargarMotivos } = useSalidaStore();
  ...
  const validateStep = (s: number): boolean => {
    ...
    if (s === 1) {
      ...
      if (!formData.estanciaDestino.trim()) e.estanciaDestino = 'El nombre de la estancia es requerido';
    ...
    if (s === 2) {
      items.forEach((item, i) => {
        if (!item.categoriaId) e[`cat_${i}`] = 'Selecciona una categoría';
        if (!item.cantidad || Number(item.cantidad) <= 0) e[`qty_${i}`] = 'Ingresa una cantidad válida';
      });
    }
    setErrors(e);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await crear({
        ...
        NombreEstanciaSalida: formData.estanciaDestino,
      ...
      Alert.alert('Éxito', 'Salida registrada', [{ text: 'OK', onPress: () => router.back() }]);
    } catch {
  ...
  const renderStep1 = () => (
    <View>
      ...
      />
      <Input
        label="Nombre Estancia Destino"
        value={formData.estanciaDestino}
        onChangeText={setField('estanciaDestino')}
        placeholder="Nombre de la estancia destino"
        error={errors.estanciaDestino}
      />
      <Select
        label="Propietario"
